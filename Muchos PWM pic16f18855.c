#include <Muchos PWM pic16f18855.h>
...
#INT_TIMER0
void interrupt()   // cada 50us entra
{
      ...
      {
             if(count1 == DC1 || count1 == DC1+1 || count1 == DC1+2 || count1 == DC1+3 || count1 == DC1-1 || count1 == DC1-2 || count1 == DC1-3)  
             //if((count1 < DC1+3 && count1 > DC1-3)) 
             {
                  output_bit(pin_b1,0);
             }
             if(count1 == DC0 || count1 == DC0+1 || count1 == DC0+2 || count1 == DC0+3 || count1 == DC0-1 || count1 == DC0-2 || count1 == DC0-3) 
            // if((count1 < DC0+3 && count1 > DC0-3)) 
             {
                  output_bit(pin_b0,0);
             }
              count1++;
      ...
      {
             if(count2 < DC3+10 && count2 > DC3-10) 
             {
                  output_bit(pin_b3,0);
             }
             if(count2 == DC2 || count2 == DC2+1 || count2 == DC2+2 || count2 == DC2+3 || count2 == DC2-1 || count2 == DC2-2 || count2 == DC2-3) 
             //if(count2 < DC2+10 && count2 > DC2-10) 
             {
                  output_bit(pin_b2,0);
             }
              count2++;
...
void main()
{
   ...
   set_timer0(6);                  // desborda en 50us // cada tic es de 200ns (256 tics) 
...
void ejecutar_estado_actual()
{
      set_adc_channel(0);
      delay_us(100);
      read0 = read_adc(ADC_START_AND_READ);    // valor read0 flotante
      DC0 = (read0 * (200./1024.));            // valor DC0 entero
