...
void  TIMER0_isr(void)   // entra cada 6us
{
      ...
      /////////////////////
                ...
                output_bit(pin_a5,0);
                output_bit(pin_c1,1);  // AVANCE_RUEDA_DERECHA. POR DEFAULT
             
                output_bit(pin_b2,0);
                output_bit(pin_b1,1);  // AVANCE_RUEDA_IZQUIERDA.  POR DEFAULT
     ...
     ///////////////////////////
      ...
      else if (count1 < 50)              // duty==>   (50<-->100%) (25<-->50%) (10<-->50%) 
      {
             ...
             {
             ///////////////////
                         output_bit(pin_a5,0);   // RETROCEDE_RUEDA_DERECHA
                         output_bit(pin_c1,0);   // AVANCE_RUEDA_DERECHA
              }
             ////////////////
             ...
             {
             /////////////////// 
                         output_bit(pin_b2,0);   // RETROCEDE_RUEDA_IZQUIERDA
                         output_bit(pin_b1,0);   // AVANCE_RUEDA_IZQUIERDA
               }
             ////////////////
...
{
   // INICIALIZAR SALIDAS //
output_bit(pin_d3,0);   // ENABLE_RUEDA_IZQUIERDA
output_bit(pin_b1,0);   // AVANCE_RUEDA_IZQUIERDA
output_bit(pin_b2,0);   // RETROCESO_RUEDA_IZQUIERDA

output_bit(pin_c0,0);   // ENABLE_RUEDA_DERECHA
output_bit(pin_c1,0);   // AVANCE_RUEDA_DERECHA
output_bit(pin_a5,0);   // RETROCESO_RUEDA_DERECHA

output_bit(pin_d1,0);   // BUZZER
////
     ...
     /*
      ...
      while(input(pin_a1) == 0)     // mientras el sensor este desactivado, mueve la rueda derecha hasta activar
            ...
            //     verificar_choque();     // verifica a ver si se levanto el bit de choque
                  output_bit(pin_c0,1);   // ENABLE_RUEDA_DERECHA
      ...
      output_bit(pin_c0,0);   // DISABLE_RUEDA_DERECHA 
      duty_derecha = 0;    // LIMPIAR EL DUTY_DERECHA PARA QUE VUELVA A COMENZAR EN 0;
       delay_ms(150);  // tiempo de inercia para la rueda derecha
      ...
      output_bit(pin_d3,0);   // DISABLE_RUEDA_IZQUIERDA  
      duty_izquierda = 0;    // LIMPIAR EL DUTY_IZQUIERDA PARA QUE VUELVA A COMENZAR EN 0;
          delay_ms(150);  // tiempo de inercia para la rueda izquierda      
*/    
   }
...
void rampa_creciente_derecha()
{
   if(duty_derecha == 49)     // SI LLEGA AL 90% DEL DC% QUEDESE ALLI.
   {
         ;
   }
...
void rampa_decreciente_derecha()
{
   if(duty_derecha == 45 || (duty_derecha > 0 && duty_derecha < 45))
   {
        duty_derecha--;
        delay_ms(1); 
   }
...
void rampa_creciente_izquierda()
{
   if(duty_izquierda == 45)
   {
         ;
   }
   else if (duty_izquierda < 45)
   {
        duty_izquierda++;
        delay_ms(50);
   }
...
void rampa_decreciente_izquierda()
{
    if(duty_izquierda == 45 || (duty_derecha > 0 && duty_derecha < 45))
   {
        duty_izquierda--;
        delay_ms(1); 
   }
...
void verificar_choque()    // si se levanta alguna bandera de choque, ejecuta giro hacia atras, dependiendo de la bandera.
{
      ...
      f_buzzer = 500; ////// colocamos 333.3 khz mientras retrocede
           ...
           duty_izquierda = 25;
           delay_us(300);
           output_bit(pin_b1,1);   // ENABLE_RUEDA_IZQUIERDA
           delay_ms(400);          // ADELANTA LA RUEDA IZQUIERDA CON DUTY DE 50% POR 0.4s
           output_bit(pin_b2,0);   // DISABLE_RUEDA_IZQUIERDA
      ...
      f_buzzer = 500; ////// colocamos 333.3 khz mientras retrocede
           ...
           {
               output_bit(pin_c0,0);   // DISABLE_RUEDA_DERECHA
                output_bit(pin_c0,0);   // DISABLE_RUEDA_IZQUIERDA
               while(1)
               {
                      //Buzzer(apagado, musica graciosa);         //   BUCLE INFINITO REPRODUCIENDO MUSICA
                        //Buzzer(apagado, musica graciosa);         //   BUCLE INFINITO REPRODUCIENDO MUSICA
